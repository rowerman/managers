...
#define PASS    1
#define NPASS   0
#define change_file "mode_5.txt"
#define rule_file_1 "file_black_5.txt"
#define rule_file_2 "file_white_5.txt"
...
int Get_signal(void)
{
    ...
    if (IS_ERR(file)) {
        printk(KERN_ERR "!!! hook_5 ljj_test Failed to open file : mode_1.txt\n");
        return PTR_ERR(file);
    }
    ...
    if (ret < 0) {
        printk(KERN_ERR "!!! hook_5 ljj_test Failed to read file: mode_1.txt\n");
    ...
    // 打印读取到的文件内容
    printk(KERN_INFO "!!! hook_5 ljj_test File mode_1.txt content: %s\n", buf);
...
void pre_process(char buf[])
{
    ...
    if (IS_ERR(file)) {
        printk(KERN_ERR "!!! hook_5 ljj_test Failed to open file file_white(or black)_1.txt\n");
        return PTR_ERR(file);
    }
    ...
    ret = vfs_read(file, buf, 4096, &file->f_pos);
    if (ret < 0) {
        printk(KERN_ERR "!!! hook_5 ljj_test Failed to read file\n");
        filp_close(file, NULL);
        set_fs(old_fs);
    }
    char tmp[9];
...
int time_check_black(unsigned int saddr){
    ...
    if (time_str = time_to_str(&tm)) {
    	...
    	printk("!!! hook_5 ljj_test time_str: %s",time_str);
        printk("!!! hook_5 ljj_test controlled_time: %s",controlled_time);
        printk("!!! hook_5 ljj_test buf: %s",controlled_time);
        if( controlled_time[0] == '0' && controlled_time[1] == '0' &&
            ...
            printk("!!! hook_5 ljj_test info: the packet %u -> %u is accepted",iph->saddr,iph->daddr);
            return NF_ACCEPT;
        }
            ...
            printk("!!! hook_5 ljj_test info: the packet is in the controlled_time range");
            if(time_str != NULL) kfree(time_str);
            // 判断 IP 地址是否在内网地址范围内
            if(is_in_internal_network(tmpskb)){
                ...
                printk("!!! hook_5 ljj_test info: the packet %u -> %u is dropped",iph->saddr,iph->daddr);
                return NF_DROP;
            }
        }
        else if(
...
unsigned int hook_func(void * priv,struct sk_buff *skb,const struct nf_hook_state * state){
    ...
    {
        printk("!!! hook_5 white_or_black_flag error! \n");
        return NF_DROP;
    }
