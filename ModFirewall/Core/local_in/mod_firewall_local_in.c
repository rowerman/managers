...
#define PASS    1
#define NPASS   0
#define change_file "mode_2.txt"
#define rule_file_1 "file_black_2.txt"
#define rule_file_2 "file_white_2.txt"
...
int local_in_ip_check_black(unsigned int saddr,int num_of_rules)
{
    int i;
    int droped_ip[100] = {0};
    printk("get black source ip: %u",saddr);
    printk("stored black source ip: %u",control_saddr[0]);
    for(i=0;i<num_of_rules;i++)
    {
        if(control_saddr[i] == 0)
        {
            printk("local_in_port_check_black accpeded because no ipaddr was offered");
        }
        else if(control_saddr[i] == saddr)
            {
                printk("local_in_port_check_black droped because the ipaddr was matched");
		droped_ip[i] = 1;
            }
            else 
            {
                printk("local_in_port_check accepted because the ipaddr was not matched");
            }
    }
...
int local_in_ip_check_white(unsigned int saddr,int num_of_rules)
{
    int i;
    int droped_ip[100] = {0};
    printk("get white source ip: %u",saddr);
    printk("stored white source ip: %u",control_saddr[0]);
    for(i=0;i<num_of_rules;i++)
    {
        if(control_saddr[i] !=0)
            if(control_saddr[i] == saddr)
            {
                printk("local_in_port_check_white accepted because the ipaddr was matched");
		droped_ip[i] = 1;
            }
            else 
            {
                printk("local_in_port_check_white accepted because the ipaddr was not matched");
            }
        else 
        {
            printk("local_in_port_check_white droped because no ipaddr was offered");
        }
    }
...
int local_in_check_black(unsigned int saddr,unsigned short sourport,int num_of_rules)
{
    int droped_ip[100] = {0};
    ...
    printk("get black source ip: %u",saddr);
    printk("stored black source ip: %u",control_saddr[0]);
    printk("get black source port: %hu ",sourport);
    printk("stored black source port: %hu",control_sport[0]);
    ...
    for(i=0;i<num_of_rules;i++)
    {
        ...
        {
            printk("local_in_port_check_black accepted because no port was offered");
        }
        else if(control_sport[i] == sourport)
            {
                printk("local_in_port_check_black droped because the port was matched");
                droped_port[i] = 1;
            }
            else
            {
                printk("local_in_port_check accepted because the port was not matched");
            }

        if(control_saddr[i] == 0)
        {
            printk("local_in_ip_check_black accepted because no ip was offered");
        }
        else if(control_saddr[i] == saddr)
            {
                printk("local_in_ip_check_black droped because the ip was matched");
                droped_ip[i] = 1;
            }
            else
            {
                printk("local_in_port_check accepted because the ip was not matched");
            }
    }
...
int local_in_check_white(unsigned int saddr,int num_of_rules)
{
    ...
    for(i=0;i<num_of_rules;i++)
    {
        ...
        {
            printk("local_in_address_check_white dropped because no address was offered");
            droped_ip[i] = 1;
        }
        else if(control_saddr[i] == saddr)
            {
                printk("local_in_address_check_white accepted because the address was matched");
            }
            else
            {
                printk("local_in_port_check accepted because the port was not matched");
...
int icmp_check(void)
{
   ...
   picmphdr = (struct icmphdr *)(tmpskb->data +(piphdr->ihl*4));
    ...
    if(!Signal)   //black module
    {
        if (picmphdr->type == 0)
        {
            if (local_in_ip_check_black(piphdr->saddr,num_of_rules) == MATCH)
            {
    ...
    else if(Signal)
    {
        if (picmphdr->type == 0)
        {
            if ( local_in_ip_check_white(piphdr->saddr,num_of_rules) == NPASS)
            {
...
//   printk("<0>This is an tcp packet.\n");
   ...
   if(!Signal)
   {
	if(local_in_check_black(piphdr->saddr,ptcphdr->source,num_of_rules))
        {
   ...
   {
       if (local_in_check_white(piphdr->saddr,num_of_rules) == PASS)
       {
...
//   printk("<0>This is an udp packet.\n");
   ...
   if(!Signal)
    {
	if(local_in_check_black(piphdr->saddr,pudphdr->source,num_of_rules))
        {
