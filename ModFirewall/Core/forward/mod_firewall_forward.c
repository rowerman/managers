...
#define PASS    1
#define NPASS   0
#define change_file "mode_3.txt"
#define rule_file_1 "file_black_3.txt"
#define rule_file_2 "file_white_3.txt"
...
int local_out_ip_check_black(unsigned int saddr,unsigned int daddr,int num_of_rules)
{
    int i;
    int droped_ip_src[100] = {0};
    int droped_ip_dest[100] = {0};
    printk("get black dest ip: %u",daddr);
    //printk("stored black desr ip: %u",control_daddr[0]);
    for(i=0;i<num_of_rules;i++)
    {
        if(control_saddr[i] == 0)
        {
            printk("forward_saddr_check_black accpeded because no ipaddr was offered");
        }
        else if(control_saddr[i] == saddr)
            {
                printk("forward_saddr_check_black droped because the ipaddr was matched");
		droped_ip_src[i] = 1;
            }
            else 
            {
                printk("forward_saddr_check_black accepted because the ipaddr was not matched");
            }
    }
    ...
    for(i=0;i<num_of_rules;i++)
    {
        ...
        {
            printk("forward_daddr_check_black accpeded because no ipaddr was offered");
        }
        else if(control_daddr[i] == daddr)
            {
                printk("forward_daddr_check_black droped because the ipaddr was matched");
		droped_ip_dest[i] = 1;
            }
            else 
            {
                printk("forward_daddr_check accepted because the ipaddr was not matched");
            }        
    }

    for(i=0;i<num_of_rules;i++)
    {
	if(droped_ip_src[i] == 1 && droped_ip_dest[i] == 1);
    {
        printk("forward package was droped because saddr or daddr was matched");
	    return MATCH;
    }

    }
    printk("forward package was accepted because saddr and daddr were not matched");
    return NMATCH;
}

int local_out_ip_check_white(unsigned int saddr,unsigned int daddr,int num_of_rules)
{
    int i;
    int droped_ip_src[100] = {0};
    int droped_ip_dest[100] = {0};
    ...
    {
        if(control_saddr[i] !=0)
            if(control_saddr[i] == saddr)
            {
                printk("forward_daddr_check_white accepted because the ipaddr was matched");
		        droped_ip_src[i] = 1;
            }
            else 
            {
                printk("forward_daddr_check_white accepted because the ipaddr was not matched");
            }
        else 
        {
            printk("forward_daddr_check_white droped because no ipaddr was offered");
        }
    }
    for(i=0;i<num_of_rules;i++)
    {
...
int local_out_check_black(unsigned int saddr,unsigned short srcport,unsigned int daddr,unsigned short dstport,int num_of_rules)
{
    int droped_ip_src[100] = {0};
    int droped_port_src[100] = {0};
    int droped_ip_dest[100] = {0};
    int droped_port_dest[100] = {0};

    int i;
    printk("get black source ip: %u",saddr);
    printk("get black source port: %hu",srcport);
    printk("get black dest ip: %u",daddr);
    ...
    for(i=0;i<num_of_rules;i++)
    {
        ...
        {
            printk("forard_desport_check_black accepted because no port was offered");
        }
        else if(control_dport[i] == dstport)
            {
                printk("forard_desport_check_black droped because the port was matched");
                droped_port_dest[i] = 1;
            }
            else
            {
                printk("forard_desport_check accepted because the port was not matched");
            }
        //目的IP
        if(control_daddr[i] == 0)
        {
            printk("forward_saddr_check_black accepted because no ip was offered");
        }
        else if(control_daddr[i] == daddr)
            {
                printk("forward_saddr_check_black droped because the ip was matched");
                droped_ip_dest[i] = 1;
            }
            else
            {
                printk("forward_saddr_check accepted because the ip was not matched");
            }

        //源端口
        if(control_sport[i]==0)  //黑名单下，端口为0应默认放行
        {
            printk("forard_srcport_check_black accepted because no port was offered");
        }
        else if(control_sport[i] == srcport)
            {
                printk("forard_srcport_check_black droped because the port was matched");
                droped_port_src[i] = 1;
            }
            else
            {
                printk("forard_srcport_check accepted because the port was not matched");
            }  
        //源IP
        if(control_saddr[i] == 0)
        {
            printk("forward_daddr_check_black accepted because no ip was offered");
        }
        else if(control_saddr[i] == saddr)
            {
                printk("forward_daddr_check_black droped because the ip was matched");
                droped_ip_dest[i] = 1;
            }
            else
            {
                printk("forward_daddr_check accepted because the ip was not matched");
            }      
    }

    for(i=0;i<num_of_rules;i++)
    {
        if(droped_ip_src[i] == 1 && droped_port_src[i] == 1 && droped_ip_dest[i] == 1 && droped_port_dest[i] == 1)
	    {
...
int local_out_check_white(unsigned int saddr,unsigned int daddr,int num_of_rules)
{
    int droped_ip_src[100] = {0};   //0代表PASS，1代表NPASS
    int droped_ip_dest[100] = {0};
    ...
    for(i=0;i<num_of_rules;i++)
    {
        if(control_saddr[i]==0)  //白名单下，端口为0应默认丢弃
        {
            printk("forward_saddr_check_white droped because no ip was offered");
            droped_ip_src[i] = 1;
        }
        else if(control_saddr[i] == saddr)
            {
                printk("forward_saddr_check_white accped because the ip was matched");
            }
            else
            {
                printk("forward_saddr_check_white droped because the ip was not matched");
                droped_ip_src[i] = 1;
            }

        if(control_daddr[i]==0)  //白名单下，端口为0应默认丢弃
        {
            printk("forward_daddr_check_white droped because no ip was offered");
            droped_ip_dest[i] = 1;
        }
        else if(control_daddr[i] == daddr)
            {
                printk("forward_daddr_check_white accped because the ip was matched");
            }
            else
            {
                printk("forward_daddr_check_white droped because the ip was not matched");
                droped_ip_dest[i] = 1;
            }
    ...
    for(i=0;i<num_of_rules;i++)
    {
        if(droped_ip_src[i] == 0 && droped_ip_dest[i] == 0)
	{
	    printk("white package was accepted!!!!!!");
            return PASS;
	}
...
int icmp_check(void)
{
   ...
   picmphdr = (struct icmphdr *)(tmpskb->data +(piphdr->ihl*4));
    ...
    if(!Signal)   //black module
    {
        if (picmphdr->type == 0)
        {
            if (local_out_ip_check_black(piphdr->daddr,piphdr->saddr,num_of_rules) == MATCH)
            {
        ...
        if (picmphdr->type == 8){
            if (local_out_ip_check_black(piphdr->saddr,piphdr->daddr,num_of_rules) == MATCH)
            {
                printk("black module, an ICMP packet is denied! \n");
                return NF_DROP;
            }
    ...
    else if(Signal)
    {
        if (picmphdr->type == 0)
        {
            if ( local_out_ip_check_white(piphdr->daddr,piphdr->saddr,num_of_rules) == NPASS)
            {
        ...
        if (picmphdr->type == 8){
            if ( local_out_ip_check_white(piphdr->saddr,piphdr->daddr,num_of_rules) == NPASS)
            {
                printk("white module, an ICMP packet is denied! \n");
                return NF_DROP;
            }
...
//   printk("<0>This is an tcp packet.\n");
   ...
   if(!Signal)
   {
	if(local_out_check_black(piphdr->saddr,ptcphdr->source,piphdr->daddr,ptcphdr->dest,num_of_rules))
        {
   ...
   {
       if (local_out_check_white(piphdr->saddr,piphdr->daddr,num_of_rules) == PASS)
       {
...
//   printk("<0>This is an udp packet.\n");
   ...
   if(!Signal)
    {
	if(local_out_check_black(piphdr->saddr,pudphdr->source,piphdr->daddr,pudphdr->dest,num_of_rules) == PASS)
        {
...
static int __init initmodule(void)
{
    ...
    myhook.hooknum=NF_INET_FORWARD;
    myhook.pf=PF_INET;
