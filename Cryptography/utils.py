...
def pad_and_reshape(binary_string, target_length, image_shape):
    # 补足二进制字符串到指定长度
    padding_needed = target_length - len(binary_string)
    if padding_needed > 0:
        binary_string = binary_string + '0' * padding_needed

    # 将二进制字符串转换为numpy数组
    binary_array = np.array(list(binary_string), dtype=np.uint8)

    # 将一维数组变形为二维数组
    binary_array = binary_array.reshape(image_shape)
