...
from PyQt5.QtCore import QTimer, QDateTime, Qt, QThread, pyqtSignal, QFile, QTextStream
import sys, os
...
from GUI import Gui
from funcs import SymEnc, AsyEnc, Signature, WaterMarking
from funcs.messages import intro
...
class EmbedImgThread(QThread):
    taskFinished = pyqtSignal(str, str, str)

    def __init__(self, imgPath, wm_path, output_path):
        super().__init__()
        self.imgPath = imgPath
    ...
    def run(self):
        outputPath, width, height = WaterMarking.embed_img(self.imgPath, self.wm_path, self.output_path)
        self.taskFinished.emit(outputPath, str(width), str(height))
        
class GetImgThread(QThread):
    taskFinished = pyqtSignal(str)

    def __init__(self, imgPath, output_path,width, height):
        super().__init__()
        self.imgPath = imgPath
        self.output_path = output_path
    ...
    def run(self):
        outputPath = WaterMarking.get_img(self.imgPath, self.output_path,self.width, self.height)
        self.taskFinished.emit(outputPath)
...
class MyMainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.label.setScaledContents(True)  # 设置scaledContents属性为True
        pixmap = QtGui.QPixmap('./pictures/logo.png')
        pixmap = pixmap.scaled(self.ui.label.width(), self.ui.label.height(), Qt.KeepAspectRatio)
        ...
        # 嵌入水印
        self.ui.pushButton_15.clicked.connect(lambda: self.ui.stackedWidget.setCurrentIndex(14))
        self.ui.pushButton_95.clicked.connect(lambda: self.ui.stackedWidget_12.setCurrentIndex(0))
        self.ui.pushButton_96.clicked.connect(lambda: self.ui.stackedWidget_12.setCurrentIndex(1))
        # 帮助文档
        self.ui.pushButton_16.clicked.connect(lambda: self.ui.stackedWidget.setCurrentIndex(15))
        # 关于界面
        self.ui.pushButton_17.clicked.connect(lambda: self.ui.stackedWidget.setCurrentIndex(16))
        # 关于我
        self.ui.pushButton_18.clicked.connect(lambda: self.ui.stackedWidget.setCurrentIndex(17))
        ...
        # 嵌入图片水印
        self.ui.pushButton_105.clicked.connect(lambda: self.browse_file(self.ui.lineEdit_47))
        self.ui.pushButton_81.clicked.connect(lambda: self.browse_file(self.ui.lineEdit_31))
        self.ui.pushButton_106.clicked.connect(lambda: self.browse_folder(self.ui.lineEdit_48))
        self.ui.pushButton_107.clicked.connect(lambda: self.browse_file(self.ui.lineEdit_51))
        self.ui.pushButton_108.clicked.connect(lambda: self.browse_folder(self.ui.lineEdit_50))
        ...
        self.ui.lineEdit_47.textChanged.connect(lambda: self.showImage(self.ui.lineEdit_47,self.ui.label_155))
        self.ui.lineEdit_31.textChanged.connect(lambda: self.showImage(self.ui.lineEdit_31, self.ui.label_120))
        self.ui.lineEdit_51.textChanged.connect(lambda: self.showImage(self.ui.lineEdit_51, self.ui.label_162))
        self.ui.pushButton_104.clicked.connect(lambda: self.Embed_image(self.ui.lineEdit_47,self.ui.lineEdit_48,
                                                                        self.ui.lineEdit_31))
        self.ui.pushButton_109.clicked.connect(lambda: self.Get_Image(self.ui.lineEdit_51,self.ui.lineEdit_50,
                                                                     self.ui.lineEdit_30,self.ui.lineEdit_52))
        
        # 帮助文档
        self.ui.pushButton_82.clicked.connect(lambda: self.show_intro(self.ui.pushButton_82))
        self.ui.pushButton_83.clicked.connect(lambda: self.show_intro(self.ui.pushButton_83))
        self.ui.pushButton_84.clicked.connect(lambda: self.show_intro(self.ui.pushButton_84))
        self.ui.pushButton_85.clicked.connect(lambda: self.show_intro(self.ui.pushButton_85))
        self.ui.pushButton_86.clicked.connect(lambda: self.show_intro(self.ui.pushButton_86))
        self.ui.pushButton_87.clicked.connect(lambda: self.show_intro(self.ui.pushButton_87))
        self.ui.pushButton_88.clicked.connect(lambda: self.show_intro(self.ui.pushButton_88))
        self.ui.pushButton_97.clicked.connect(lambda: self.show_intro(self.ui.pushButton_97))
        self.ui.pushButton_110.clicked.connect(lambda: self.show_intro(self.ui.pushButton_110))
        self.ui.pushButton_111.clicked.connect(lambda: self.show_intro(self.ui.pushButton_111))
        self.ui.pushButton_112.clicked.connect(lambda: self.show_intro(self.ui.pushButton_112))
        self.ui.pushButton_113.clicked.connect(lambda: self.show_intro(self.ui.pushButton_113))
        self.ui.pushButton_114.clicked.connect(lambda: self.show_intro(self.ui.pushButton_114))
        self.ui.pushButton_115.clicked.connect(lambda: self.show_intro(self.ui.pushButton_115))
        self.ui.pushButton_116.clicked.connect(lambda: self.show_intro(self.ui.pushButton_116))
    ...
    def showtime(self,StartTime):
        ...
        text = "你已经工作了\n" + str(hours) + "小时"+ str(minutes) + "分钟"+ str(workingTime) + "秒"
    ...
    def show_intro(self,Button):
        index = Button.text()
        Intro = intro[index]
        QMessageBox.information(self, "提示", Intro)
        return
    ...
    def Embed_image(self,LineEdit1,LineEdit2,LineEdit3):
        if LineEdit1.text() == '':
            QMessageBox.information(self, "提示", "请输入待嵌入图片的路径！")
            return
        if LineEdit2.text() == '':
            QMessageBox.information(self, "提示", "请输入待嵌入图片的保存路径！")
            return
        if LineEdit3.text() == '':
            QMessageBox.information(self, "提示", "请输入水印图片的路径！")
            return
        ...
        if ext.lower() in ['.jpg', '.jpeg', '.png', '.bmp', '.gif']:
            if imghdr.what(imgPath) == None:
                QMessageBox.information(self, "提示", "待嵌入的不是图片！")
                return
        else:
            QMessageBox.information(self, "提示", "待嵌入的不是图片！")
            return
        
        wmPath = LineEdit3.text()
        _, ext = os.path.splitext(wmPath)
        if ext.lower() in ['.jpg', '.jpeg', '.png', '.bmp', '.gif']:
            if imghdr.what(wmPath) == None:
                QMessageBox.information(self, "提示", "水印图片上传错误！")
                return
        else:
            QMessageBox.information(self, "提示", "水印图片上传错误！")
            return
        output_path = LineEdit2.text()
        thread = EmbedImgThread(imgPath, wmPath, output_path)
        thread.taskFinished.connect(self.on_embed_img_finished)  # 当任务完成时，关闭对话框
        thread.start()
    ...
    def on_embed_img_finished(self, output_path, width, height):
        self.ui.label_156.setScaledContents(True)  # 设置scaledContents属性为True
        pixmap = QtGui.QPixmap(output_path)
        pixmap = pixmap.scaled(self.ui.label_156.width(), self.ui.label_156.height(), Qt.KeepAspectRatio)
        self.ui.label_156.setPixmap(pixmap)
        
        self.ui.lineEdit_29.setText(width)
        self.ui.lineEdit_49.setText(height)
        self.ui.lineEdit_48.setText(output_path)
        self.messageBox.accept()  # 关闭对话框
        
    def Get_Image(self,LineEdit1,LineEdit2,LineEdit3,LineEdit4):
        if LineEdit1.text() == '':
            QMessageBox.information(self, "提示", "请输入待提取图片的路径！")
            return
        if LineEdit2.text() == '':
            QMessageBox.information(self, "提示", "请输入提取出水印的保存路径！")
            return
        if LineEdit3.text() == '':
            QMessageBox.information(self, "提示", "请输入水印图片的宽度！")
            return
        if LineEdit4.text() == '':
            QMessageBox.information(self, "提示", "请输入水印图片的高度！")
            return
        imgPath = LineEdit1.text()
        _, ext = os.path.splitext(imgPath)
        if ext.lower() in ['.jpg', '.jpeg', '.png', '.bmp', '.gif']:
        ...
        output_path = LineEdit2.text()
        width = LineEdit3.text()
        height = LineEdit4.text()
        
        thread = GetImgThread(imgPath, output_path, width, height)
        thread.taskFinished.connect(self.GetImage_finished)  # 当任务完成时，关闭对话框
        thread.start()
    ...
    def GetImage_finished(self,output_path):
        self.ui.lineEdit_50.setText(output_path)
        self.ui.label_117.setScaledContents(True)  # 设置scaledContents属性为True
        pixmap = QtGui.QPixmap(output_path)
        pixmap = pixmap.scaled(self.ui.label_117.width(), self.ui.label_117.height(), Qt.KeepAspectRatio)
        self.ui.label_117.setPixmap(pixmap)
        self.messageBox.accept()
